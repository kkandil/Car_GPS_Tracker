...
unsigned int invlTx = 30;
byte txData[] = {0x02,0x01,0x0D,0x55,0x55,0x55,0x55,0x55};

//byte txData[] = {0x02,0x01,0x00,0x00,0x00,0x00,0x00,0x00};
//byte txData2[] = {0x02,0x01,0x0C,0x00,0x00,0x00,0x00,0x00};
...
#define CAN0_INT 8                           /* Set INT to pin 2 (This rarely changes)   */
...
void loop(){
  ...
  if(!digitalRead(CAN0_INT)){                         // If CAN0_INT pin is low, read receive buffer
    ...
    CAN0.readMsgBuf(&rxID, &dlc, rxBuf);             // Get CAN data 
     if( rxID == 0x7E9 )
     { 
        if( rxBuf[3] >= 0 && rxBuf[3] <=255 )
        {
          new_vehicle_speed = (int) rxBuf[3];
    
          if( new_vehicle_speed != vehicle_speed )
          {
            vehicle_speed = new_vehicle_speed;
            display.showNumberDec(vehicle_speed, false); 
          }
        }
        //sprintf(msgString, "Standard ID: 0x%.3lX, PID: 0x%.2X, S: 0x%.2X", rxID, rxBuf[2], rxBuf[3]);
